...
#     cv2.imshow('frame', frame_rgb)
    ...
    if cv2.waitKey(1) & 0xFF == ord('t') and current_capture_count < captures_per_plant:
...
#                 print(bottomright)
                ...
                # Update count of healthy, unhealthy, and flowers 
        ...
        # Calculate the average results for the current plant
        if len(results_per_plant) == 5:  # After capturing 5 images for the current plant
            avg_results = [round(sum(x) / 5) for x in zip(*results_per_plant)]
            with open(CSV_FILENAME, 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([f"Plant {plant_number:02d}"] + avg_results)

            plant_number += 1
            results_per_plant.clear()  # Prepare for the next plant

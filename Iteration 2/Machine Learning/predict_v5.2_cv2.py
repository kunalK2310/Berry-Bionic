...
#video_object = cv2.VideoCapture("nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)3280, height=(int)2464,format=(string)NV12, framerate=(fraction)30/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)RGB ! appsink")
video_object = cv2.VideoCapture("nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)1280, height=(int)720,format=(string)NV12, framerate=(fraction)30/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! appsink", cv2.CAP_GSTREAMER)
...
while True:
    ...
    # Wait for 't' press to start capturing images for the current plant
    if cv2.waitKey(1) & 0xFF == ord('t'):
        for _ in range(captures_per_plant):
            ret, frame = video_object.read()
            if not ret:
                break  # If there's an issue capturing the frame, exit the loop
    ...
    #                 print(bottomright)
            ...
            #time.sleep(0.01)  # 10ms delay between captures

        # After capturing the set number of images for the plant, process results
        if len(results_per_plant) == captures_per_plant:
